...
function generateSphereOBJ(rings = 12, slices = 18, radius = 0.5) {
  ...
  for (let r = 0; r <= rings; r++) {
    for (let s = 0; s <= slices; s++) {
      const nx =
        Math.sin((r / rings) * Math.PI) * Math.cos((s / slices) * 2 * Math.PI);
      const ny = Math.cos((r / rings) * Math.PI);
      const nz =
        Math.sin((r / rings) * Math.PI) * Math.sin((s / slices) * 2 * Math.PI);
...
function render() {
  ...
  gl.uniform3fv(uniforms.uLightColor, [1.0, 0.95, 0.85]);
...
async function init() {
  ...
  try {
    ...
    console.warn(
      "   Certifique-se de rodar via servidor HTTP (ex: npx serve .)",
    );
  }
  ...
  // Inicializa o menu passando a função de callback do Game Loop corretamente
  initMainMenu(() => {
    paused = false;
